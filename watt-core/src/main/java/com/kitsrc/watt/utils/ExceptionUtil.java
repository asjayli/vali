...
import com.kitsrc.watt.utils.function.RunnableWithException;
...
public class ExceptionUtil {
     ...
     * @return
     */
    public static Throwable recursionException(Throwable throwable) {
        ...
        if (ObjectUtil.isEmpty(cause)) {
            ...
            // 递归入口
            return recursionException(cause);
        }
    }

    /**
     * 递归获取最内层的  InvocationTargetException 异常对象
     *
     * @param ite
     * @return
     */
    public static InvocationTargetException recursionInvocationTargetException(InvocationTargetException ite) {
        ...
        if (ObjectUtil.isEmpty(targetException)) {
            ...
            if (targetException instanceof InvocationTargetException) {
                // 递归入口
                return recursionInvocationTargetException((InvocationTargetException) targetException);
            } else {
                // 递归终止标记
                return ite;
            }
    ...
    public static String stringifyException(final Throwable e) {
        ...
        try {
            ...
            return e.getClass()
                    .getName() + " (error while printing stack trace)";
        }
    }
     ...
     *
     * <p>See {@link ExceptionUtil#isJvmFatalError(Throwable)} for a list of fatal JVM errors.
     ...
     * See {@link ExceptionUtil#isJvmFatalError(Throwable)} for a definition of fatal errors.
     *
    ...
    /**
     * Rethrows the given {@code Throwable}, if it represents an error that is fatal to the JVM
     * or an out-of-memory error. See {@link ExceptionUtil#isJvmFatalError(Throwable)} for a
