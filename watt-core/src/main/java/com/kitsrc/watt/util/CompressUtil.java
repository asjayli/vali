...
public class CompressUtil {
    ...
    public static byte[] compress(final byte[] src) throws IOException {
        byte[] result;
        @Cleanup ByteArrayOutputStream bos = new ByteArrayOutputStream(src.length);
        @Cleanup GZIPOutputStream gos = new GZIPOutputStream(bos);
        try {
            gos.write(src);
            gos.finish();
    ...
    public static byte[] uncompress(final byte[] src) throws IOException {
        ...
        @Cleanup ByteArrayInputStream bis = new ByteArrayInputStream(src);
        @Cleanup GZIPInputStream iis = new GZIPInputStream(bis);
        @Cleanup ByteArrayOutputStream bos = new ByteArrayOutputStream(src.length);
