package com.kitsrc.watt.net.clients;

import com.kitsrc.watt.utils.IOUtil;
import com.kitsrc.watt.utils.StringUtil;
import com.kitsrc.watt.net.clients.ahc.ApacheIdleConnectionCleaner;
import com.kitsrc.watt.net.clients.ahc.CallBack;
import com.kitsrc.watt.net.clients.ahc.HttpClientConfig;
import com.kitsrc.watt.net.clients.ahc.IHttpClient;
import com.kitsrc.watt.net.clients.ahc.*;
import com.kitsrc.watt.net.clients.ahc.exception.ClientException;
import com.kitsrc.watt.net.utils.ProxyEnvUtil;
...
public class ApacheHttpClient extends IHttpClient {
    ...
    private HttpHost calcProxy(HttpRequest apiReq) throws MalformedURLException, ClientException {
        boolean needProxy = HttpUtil.needProxy(new URL(apiReq.getSysUrl()).getHost(), clientConfig.getNoProxy(), ProxyEnvUtil.getNoProxy());
        if (!needProxy) {
            return null;
        }
        ...
        if ("https".equalsIgnoreCase(url.getProtocol())) {
            proxy = HttpUtil.getApacheProxy(clientConfig.getHttpsProxy(), ProxyEnvUtil.getHttpsProxy(), apiReq);
        } else {
            proxy = HttpUtil.getApacheProxy(clientConfig.getHttpProxy(), ProxyEnvUtil.getHttpProxy(), apiReq);
        }
        return proxy;
    }
