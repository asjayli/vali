package com.vali.net.http.common;
...
public class Utils {
    ...
    private static final List<String> SPECIAL_ENTITIY = Arrays.asList(ENTITY_STRING, ENTITY_JSON, ENTITY_BYTES, ENTITY_FILE,
            ...
            for (Entry<String, Object> entry : map.entrySet()) {
                if (SPECIAL_ENTITIY.contains(entry.getKey())) {//判断是否在之中
                    ...
                    if (ENTITY_STRING.equals(entry.getKey())) {//string
                        ...
                        if (File.class.isAssignableFrom(entry.getValue()
                                                             .getClass())) {
...
//							builder.addPart(String.valueOf(map.get(ENTITY_MULTIPART+".name")) + count++, fileBody);
                        ...
                        Iterator<Entry<String, Object>> iterator = m.entrySet()
                                                                    .iterator();
                        // 发送的数据
